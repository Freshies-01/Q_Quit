...
public interface SeparationApplicationRepository extends JpaRepository<SeparationApplication, Long> {
    ...
    @Query("SELECT app FROM SeparationApplication app WHERE app.status != 'CLOSED_BY_HR' AND app.hr.employee.user.login = ?#{principal.username} OR app.employee.user.login = ?#{principal.username} OR app.fr.employee.user.login = ?#{principal.username}")
