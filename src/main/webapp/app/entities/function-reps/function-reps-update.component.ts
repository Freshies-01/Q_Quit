...
export class FunctionRepsUpdateComponent implements OnInit {
    ...
    save() {
        ...
        if (this.functionReps.id !== undefined) {
            this.subscribeToSaveResponse(this.functionRepsService.update(this.functionReps));
        } else {
            this.subscribeToSaveResponse(this.functionRepsService.create(this.functionReps));
        }
    }

    private subscribeToSaveResponse(result: Observable<HttpResponse<IFunctionReps>>) {
        result.subscribe((res: HttpResponse<IFunctionReps>) => this.onSaveSuccess(), (res: HttpErrorResponse) => this.onSaveError());
    }
