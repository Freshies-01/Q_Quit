...
export class HrRepsDeleteDialogComponent {
    ...
    constructor(private hrRepsService: HrRepsService, public activeModal: NgbActiveModal, private eventManager: JhiEventManager) {}
...
export class HrRepsDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ hrReps }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(HrRepsDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
