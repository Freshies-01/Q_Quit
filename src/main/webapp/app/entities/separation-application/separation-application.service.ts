...
export class SeparationApplicationService {
    ...
    private convertDateFromClient(separationApplication: ISeparationApplication): ISeparationApplication {
        const copy: ISeparationApplication = Object.assign({}, separationApplication, {
            dateOfLeave:
                separationApplication.dateOfLeave != null && separationApplication.dateOfLeave.isValid()
                    ? separationApplication.dateOfLeave.format(DATE_FORMAT)
                    : null,
            dateSumbitted:
                separationApplication.dateSumbitted != null && separationApplication.dateSumbitted.isValid()
                    ? separationApplication.dateSumbitted.format(DATE_FORMAT)
                    : null,
            dateCompleted:
                separationApplication.dateCompleted != null && separationApplication.dateCompleted.isValid()
                    ? separationApplication.dateCompleted.format(DATE_FORMAT)
                    : null,
            dateApproved:
                separationApplication.dateApproved != null && separationApplication.dateApproved.isValid()
    ...
    private convertDateFromServer(res: EntityResponseType): EntityResponseType {
        res.body.dateOfLeave = res.body.dateOfLeave != null ? moment(res.body.dateOfLeave) : null;
        res.body.dateSumbitted = res.body.dateSumbitted != null ? moment(res.body.dateSumbitted) : null;
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((separationApplication: ISeparationApplication) => {
            separationApplication.dateOfLeave =
                separationApplication.dateOfLeave != null ? moment(separationApplication.dateOfLeave) : null;
            separationApplication.dateSumbitted =
                separationApplication.dateSumbitted != null ? moment(separationApplication.dateSumbitted) : null;
            separationApplication.dateCompleted =
                separationApplication.dateCompleted != null ? moment(separationApplication.dateCompleted) : null;
            separationApplication.dateApproved =
                separationApplication.dateApproved != null ? moment(separationApplication.dateApproved) : null;
        });
        return res;
    }
}
